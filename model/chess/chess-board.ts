import {
    ...
    MoveType,
    SafeSquares
} from "@/interface/chess-props";
import Bishop from "./bishop";
import FENConverter from "./fen-converter";
...
export default class ChessBoard {
  private chessBoard: ChessBoardProps;
  private readonly chessBoardSize: number = 8;
  private _playerColor = Color.White;
  private _safeSquares: SafeSquares;
  private _lastMove: LastMove | undefined;
  private _checkState: CheckState = { checked: false };
  ...
  public get chessBoardView(): BoardView {
  ...
  public checked(playerColor: Color, checkingCurrentPosition: boolean): boolean {
              ...
              for (const { x: dx, y: dy } of piece.directions) {
                  ...
                  if (piece instanceof Pawn || piece instanceof Knight || piece instanceof King) {
                      ...
                      if (attackedPiece instanceof King && attackedPiece.color === playerColor) {
                          if (checkingCurrentPosition) this._checkState = { checked: true, x: newX, y: newY };
                          return true;
                      }
                  }
                  else {
      ...
      if (checkingCurrentPosition) this._checkState = { checked: false };
      return false;
  }
  ...
  private isPositionSafeAfterMove(prevX: number, prevY: number, newX: number, newY: number): boolean {
      ...
      const isPositionSafe: boolean = !this.checked(piece.color, false);
  ...
  private findSafeSqures(): SafeSquares {
      const safeSqures: SafeSquares = new Map<string, Coordinates[]>();
      ...
      for (let x = 0; x < this.chessBoardSize; x++) {
          for (let y = 0; y < this.chessBoardSize; y++) {
              ...
              const pieceSafeSquares: Coordinates[] = [];
  ...
  private canCastle(king: King, kingSideCastle: boolean): boolean {
      ...
      if (!(rook instanceof Rook) || rook.hasMoved || this._checkState.checked) return false;
  ...
  public movePiece(prevX: number, prevY: number, newX: number, newY: number, promotedPieceType: FENChar | null): void {
      ...
      const pieceSafeSquares: Coordinates[] | undefined = this._safeSquares.get(prevX + "," + prevY);
      ...
      this.checked(this._playerColor, true);
      ...
      console.log(this._isGameOver, this._gameOverMessage)
  }
  ...
  private startingPieceCoordsNotation(): string {
      ...
      const samePiecesCoords: Coordinates[] = [{ x: prevX, y: prevY }];
      ...
      for (let x = 0; x < this.chessBoardSize; x++) {
          for (let y = 0; y < this.chessBoardSize; y++) {
              ...
              if (piece.FENChar === currPiece.FENChar) {
                  const safeSquares: Coordinates[] = this._safeSquares.get(x + "," + y) || [];
